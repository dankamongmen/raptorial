...
// Do not store pointers to either of these types (edges or sdfactx's), as they
...
typedef struct sdfa {
	...
	sdfavtx *vtxarray; // Dynamic array of sdfavtxs
	unsigned vtxcount,vtxalloc;
} sdfa;

PUBLIC void
init_sdfactx(sdfactx *dctx,const sdfa *space){
	dctx->dfa = space;
	dctx->cur = 0;
}

sdfactx *create_sdfactx(const sdfa *space){
	sdfactx *dctx;

	if( (dctx = malloc(sizeof(*dctx))) ){
		init_sdfactx(dctx,space);
	}
	return dctx;
}

// We can't just use bsearch(3) -- see comment in augment_sdfa(); we want to
...
edge_search(const sdfavtx *node,int s){
	int max,min;
	unsigned pos;
...
PUBLIC int
augment_sdfa(sdfa **space,const char *str,void *val){
	const char *s;
	sdfavtx *cur;
...
void *match_sdfactx_char(sdfactx *dctx,int s){
	unsigned pos;
...
void *match_sdfactx_string(sdfactx *dctx,const char *str){
	while(*str){
		unsigned pos;
...
void *match_sdfactx_nstring(sdfactx *dctx,const char *s,size_t len){
	while(len--){
		unsigned pos;
...
static int
recurse_sdfa(const sdfa *d,const sdfavtx *dvtx,char *str,unsigned stroff,
	...
	for(e = 0 ; e < dvtx->setsize ; ++e){
		...
		if(recurse_sdfa(d,d->vtxarray + dvtx->set[e].vtx,str,
					stroff + 1,cb,opaq)){
			return -1;
		}
	}
...
int walk_sdfa(const sdfa *d,int (*cb)(const char *,const void *,const void *),
					const void *opaq){
	if(d){ // New sdfas get longest = 1, so needn't check that
		char str[d->longest];

		return recurse_sdfa(d,d->vtxarray,str,0,cb,opaq);
	}
	return 0;
}
